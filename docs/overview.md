...
| `POST /validate-cocoonhibernation` | Validating | CocoonHibernation CREATE | Requires `metadata.name` to equal `spec.podRef.name` (one CR per pod, named after it, so racing duplicate CREATEs collide on name uniqueness) and rejects a CR whose pod already has one — live or still terminating — so two CRs can never fight over one VM. Retargeting an existing CR is blocked by the CRD's CEL rule on `spec.podRef`. |
